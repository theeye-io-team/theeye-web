  ...
  $.backgroundVideo = function(el, options) {
    ...
    var buildVideo = function () {
      ...
      for(var i=0; i < plugin.settings.types.length; i++) {
        html += '<source src="'+'https://s3.amazonaws.com/www.theeye.io/'+plugin.settings.path+plugin.settings.filename+'.'+plugin.settings.types[i]+'" type="video/'+plugin.settings.types[i]+'" />';
      }
      html += 'bgvideo</video>';
