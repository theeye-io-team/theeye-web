...
export default () => {
  const a = window.localStorage.getItem('achievements') || {}
  let parsed = null
  try {
    parsed = JSON.parse(a) || {}
  } catch (e) {
  ...
  if (parsed.cronMaster === true) {
    return
  } else {
    parsed.cronMaster = true
    window.localStorage.setItem('achievements', JSON.stringify(parsed))
    document.body.append(new FuzzyMessage().render().el)
  }
}

const FuzzyMessage = View.extend({
  template: `<div id="fuzzy-container">
      <canvas id="fuzzy-message"></canvas>
      <div id="buffer" style="display:none;"></div>
    </div>`,
  initialize: function () {
    this.on('animation-ended', () => {
      this.canvas.classList.remove('fuzzy-fadein')
      this.canvas.classList.add('fuzzy-fadeout')
      window.setTimeout(() => this.remove(), 800)
    })
  },
  render: function () {
    this.renderWithTemplate(this)
    this.canvas = this.query('#fuzzy-message')
    this.canvas.classList.add('fuzzy-fadein')
    this.runMessage()
    // this.runMessage()
  },
  runMessage: function () {
    ...
    var Sphere = function (arg) {
      ...
      for (var i in chars) {
        ...
        this.charsMap[i] = []
        var self = this
        for (var j2 = 0; j2 < this.particleNum; j2++) {
          var redo = function () {
      ...
      this.charsMap['@'] = []
      for (var i2 = 0; i2 < this.particleNum; i2++) {
        this.charsMap['@'][i2] = {theta: 360 * Math.random(), phi: 360 * Math.random()}
      };
      this.charsMap['_'] = []
      for (var i3 = 0; i3 < this.particleNum; i3++) {
        this.charsMap['_'][i3] = {theta: 0, phi: 0}
      };

      this.veticies = []
      for (var i4 = 0; i4 < this.particleNum; i4++) {
        this.veticies[i4] = new Vertex3d({})
      };
    }
    Sphere.prototype = {
      update: function () {
    ...
    for (var i in chars) {
      charsLength++
      bufferImages[i] = new window.Image()
    ...
    var bufferDraw = function () {
      for (let i in chars) {
        ...
        document.getElementById(i).getContext('2d')
    ...
    var textSet = [
      {text: '_', sphereRadius: 120, sphereSpace: 80, unitTime: 30, time: 500},
      {text: 'CRON', sphereRadius: 100, sphereSpace: 80, unitTime: 80, time: 50},
      {text: 'MASTER', sphereRadius: 100, sphereSpace: 80, unitTime: 80, time: 1500},
      {text: 'ACHIEVED', sphereRadius: 100, sphereSpace: 80, unitTime: 200, time: 1500},
      {text: '_', sphereRadius: 120, sphereSpace: 80, unitTime: 120, time: 5000}
    ]
