...
assign(FilteredCollection.prototype, Events, {
  ...
  _onCollectionEvent: function (event, model, that, options) {
    ...
    // Whether or not we are to expect a sort event from our collection later
    var sortable = this.collection.comparator && (!options.at) && (options.sort !== false)
    var add = options.add
    ...
    if (action === 'sort' ||
          (
            propName &&
            !sortable &&
            includes([this.comparator, this.collection.comparator].concat(this._watched), propName)
          )
    ) {
      // wtf? when action === 'sort' (aka: sort event) model is always 'this'
      // and has no prop isNew. this is fubar
      // if (ordered && model.isNew) return // We'll get a sort later
      this.models = this._sortModels(this.models)
      // if we don't trigger sort on sort the collection doesn't
      // sort (kinda obvious)
